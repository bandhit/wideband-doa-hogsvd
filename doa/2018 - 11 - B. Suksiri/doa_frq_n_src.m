...
%
% Description : A Highly Efficient Wideband Two-Dimensional Direction Estimation Method with 
%               L-Shaped Microphone Array, IEICE, ????.
%
...
function [ang_x_rad_vec, ang_z_rad_vec, s_r_mat, u_r_mat, v_r_mat] = ...
    ...
    fc_x_fc_z_r_mat = zeros(n_sen, n_sen);
    for i_tim = 1: 1: n_tim
        fc_x_fc_z_r_mat = fc_x_fc_z_r_mat + (fc_x_mat(:, i_tim) * fc_z_mat(:, i_tim)');
    end
    fc_x_fc_z_r_mat = fc_x_fc_z_r_mat / n_tim;
    ...
    for i_frq = 1: 1: n_frq
        ...
        fi_x_fi_z_r_mat = zeros(n_sen, n_sen);
        fi_x_fc_z_r_mat = zeros(n_sen, n_sen);
        fc_x_fi_z_r_mat = zeros(n_sen, n_sen);
        for i_tim = 1: 1: n_tim
            fi_x_fi_z_r_mat = fi_x_fi_z_r_mat + (fi_x_mat(:, i_tim) * fi_z_mat(:, i_tim)');
            fi_x_fc_z_r_mat = fi_x_fc_z_r_mat + (fi_x_mat(:, i_tim) * fc_z_mat(:, i_tim)');
            fc_x_fi_z_r_mat = fc_x_fi_z_r_mat + (fc_x_mat(:, i_tim) * fi_z_mat(:, i_tim)');
        end
        fi_x_fi_z_r_mat = fi_x_fi_z_r_mat / n_tim;
        fi_x_fc_z_r_mat = fi_x_fc_z_r_mat / n_tim;
        fc_x_fi_z_r_mat = fc_x_fi_z_r_mat / n_tim;
        
        r_x_mat = (fi_x_fc_z_r_mat  * fc_x_fc_z_r_mat');
        r_z_mat = (fc_x_fi_z_r_mat' * fc_x_fc_z_r_mat );
        % alternative solution
        % r_x_mat = (fc_z_fi_x_r_mat' * fc_z_fc_x_r_mat);
        % r_z_mat = (fi_z_fc_x_r_mat  * fc_z_fc_x_r_mat');
        
        [u_bar_fi_mat, ~, u_bar_fc_mat] = svd(r_x_mat);
        [v_bar_fi_mat, ~, v_bar_fc_mat] = svd(r_z_mat);
        
        t_x_mat = u_bar_fc_mat(:, 1: n_rank_fi) * u_bar_fi_mat(:, 1: n_rank_fi)';
        t_z_mat = v_bar_fc_mat(:, 1: n_rank_fi) * v_bar_fi_mat(:, 1: n_rank_fi)';
        r_mat   = r_mat + (t_x_mat * fi_x_fi_z_r_mat * t_z_mat');
    end
...
%     v_2_mat = v_r_mat(2: end,     1: n_rank_fc);
    ...
    ang_x_rad_vec = acos(((cen_frq * lamb) / (2 * pi * d_sen * src_frq)) * ...
                         angle(eig_val_u_vec(sel_col_fc_vec, 1)));
	ang_z_rad_vec = acos(((cen_frq * lamb) / (2 * pi * d_sen * src_frq)) * ...
                         angle(eig_val_v_vec(sel_row_fc_vec, 1)));
end
