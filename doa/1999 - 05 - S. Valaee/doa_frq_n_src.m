...
%
% Description : S. Valaee et al., Localization of Wideband Signals using Least-squares and Total 
%               Least-squares Approaches, IEEE, 1999.
%
...
function [init_ang_x_rad_vec, init_ang_z_rad_vec] = ...
    ...
    for i_iter = 1: 1: n_iter
        ...
        for i_frq = 1: 1: n_frq
            ...
            t_x_mat = a_x_fc_mat * ((a_x_fc_mat' * a_x_fc_mat) ^ (- 1/ 2)) * ((a_x_fi_mat' * a_x_fi_mat) ^ (- 1/ 2)) * a_x_fi_mat';
            t_z_mat = a_z_fc_mat * ((a_z_fc_mat' * a_z_fc_mat) ^ (- 1/ 2)) * ((a_z_fi_mat' * a_z_fi_mat) ^ (- 1/ 2)) * a_z_fi_mat';
